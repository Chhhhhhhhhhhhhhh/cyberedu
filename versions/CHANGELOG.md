...
**重磅里程碑**：全平台九大核心安全体系 100% 完成向「原子微课与交互速查手册体系」工业级重构，课时规模从 52 讲全面跨越至 **171 讲（342 篇中英双语教材 + 342 组随堂评测）**。

### 🎓 1. 九大核心体系全量交付 (50 章 / 171 讲 / 342 篇双语教材)
- **Web 安全 (`websec`)**：10 章 39 讲，覆盖 SQLi、XSS、CSRF、SSRF、XXE、文件上传、认证会话、RCE 反序列化、API 安全、DevSecOps & WAF。
- **渗透测试 (`pentest`)**：5 章 15 讲，从 OSINT 侦察、端口指纹扫描到提权横向移动完整渗透作战链。
- **计算机网络 (`network`)**：5 章 15 讲，系统剖析 OSI/TCP/IP 栈、DNS/ARP 欺骗、HTTP/1.1~HTTP/3 演进与 Wireshark 流量取证。
- **密码学与现代应用 (`cryptography`)**：6 章 18 讲，涵盖古典密码、AES/DES、RSA/ECC、数字签名、TLS 1.3 握手与零知识证明/后量子。
- **恶意软件分析与逆向 (`malware`)**：5 章 15 讲，深入 PE 结构、动态沙箱、IDA/Ghidra 反编译与反调试对抗。
- **云原生与容器安全 (`cloudsec`)**：5 章 15 讲，全面覆盖 Docker 逃逸、K8s 集群加固、云 IAM/元数据攻击与微服务网格安全。
- **蓝队监控与应急取证 (`dfir`)**：5 章 15 讲，构建 SOC/SIEM 运营、主机痕迹取证、Volatility 内存取证与勒索处置 SOP。
- **编程基础 (`programming`)**：7 章 18 讲，安全 Python 脚本、C 内存漏洞、Bash 管道、Go 并发工具与汇编安全规范。
- **CTF 实战攻防 (`ctf-guide`)**：7 章 21 讲，PWN 栈溢出/ROP 链、逆向反反编译、Web 深度利用链、Crypto 破译与 AWD 攻防对抗体系。

### 🎯 2. 交互式随堂测验 Checkpoint 体系 (342 组自测)
- 每节微课标配 2 道单选检测题，实时交互判定正误。
- 配套详尽底层原理解析，并集成「🤖 呼叫 AI 导师」针对错题一键展开启发式教学。

### 🛠️ 3. 六大交互式安全武器库与沙箱
- **SQL 注入沙箱**：真实模拟动态 SQL 拼接利用与参数化防护效果对比。
- **XSS 实体演练场**：实时演示字符过滤、实体转义与绕过机制。
- **命令注入分析仪**：直观剖析管道分隔符截断与子进程隔离防御。
- **密码学哈希分析仪**：动态计算并高亮展示 SHA-256/MD5 雪崩效应翻转比特数。
- **五维技能雷达图**：动态量化评测个人技术强弱项，自适应推荐强化路径。
- **实验工具抽屉**：各课时专属的武器库与命令速查抽屉。

### 🎮 4. 黑客段位与成长激励引擎
- 5 阶段位演进体系（LV.1 脚本小子 → LV.2 初级黑客 → LV.3 安全专家 → LV.4 首席红蓝对抗师 → LV.5 赛博主宰）。
- 基于微课通关、CTF 攻克与编程练习的动态 EXP 算法，集成连续答题加成与成就徽章解锁。

### 🌐 5. 终端黑客美学与全量双语对齐
- 首页打字机（Terminal Typewriter）严格保持与 Hero 大标题一致的纯英文赛博终端美学，自适应绑定 171 章节数据。
- 100% 章节实现英汉双轨并行，顶部导航一键无缝切换。

### 📱 6. PWA 渐进式离线应用支持
- 新增 `manifest.json` 与 Service Worker 缓存，支持将平台作为桌面或移动端独立 Web App 安装，无网离线亦可畅读。

### 🔒 7. 严苛工业级质量保障门禁
- **自动化单元测试**：`npm test` 增至 114 项测试，全量通过。
- **CTF 逻辑校验**：`verify-ctf-solvable.js` 验证 21 道挑战数学与技术真实可解。
- **模板合规校验**：`verify-content-quality.js` 确保 342 篇微课符合标准规范（342/342 OK）。
- **零隐私泄漏**：集成 `privacy-scan.js`，严格执行脱敏安全扫描。

## v2.7.6 — 2026-08-29

**文件**: `cyberedu.html`, `script.js`, `style.css`, `i18n.js`, `scripts/history-drawer-v276.js`(新)

### 🗂 AI 历史会话：侧边窄条 → 全面板抽屉（用户反馈"小/劣质/不适配"）
- **交互形态重做**：历史会话从挤在面板左上角的 200px 窄条，改为**覆盖整个 AI 面板的抽屉层**（滑入动画、圆角贴合），对齐主流 AI 产品的会话抽屉交互
- **会话搜索**：抽屉顶部新增搜索框，按标题实时过滤
- **时间分组**：会话按"今天 / 7 天内 / 更早"分组展示，组标题中英双语
- **底部主操作**："＋ 新对话"主按钮（绿色）+ "清空全部"（两段式确认）
- 打开抽屉时自动隐藏消息区/输入区/设置页；关闭即恢复聊天；选中会话自动收起抽屉并载入
- 空状态、当前未保存会话（呼吸圆点 + 未保存徽标 + 计数）、重命名 ✎、两段式删除 ✓ 全部适配新结构
...
- 修复分组标题直接显示 i18n 键名的问题（补齐 `ai.grpToday/7d/Older` 中英文案）
- 旧侧栏 CSS 规则成为死代码（元素已改用新类名），后续清理

## v2.7.5 — 2026-08-28

**文件**: `script.js`, `style.css`, `scripts/history-redesign-v275.js`(新)

### 🎨 AI 历史侧栏视觉重设计
- 卡片式条目 + 绿色左侧强调线 + 双行布局（标题+相对时间 / 消息数+悬停操作）
- 当前对话绿色呼吸圆点标记；悬停显操作；删除两段式确认
- 清空全部精致化；浅色主题适配
- 修正早期审计对部分章节篇幅的低估（prog-03 实际 16.5K 字符等）

## v2.7.2 — 2026-08-28

**文件**: `script.js`, `i18n.js`, `style.css`, `scripts/ai-upgrade-v272.js`(新), `package.json`
### 🤖 AI 导师体验升级（用户反馈驱动）
- **中止生成**：流式输出时发送按钮变为红色 ■ 停止键（AbortController），点击立即中止并<strong>保留已生成的部分内容</strong>
- **消息操作条**：每条 AI 回答完成后附“复制全文 / ↻ 重新生成”按钮；出错气泡附重试按钮
- **代码块复制 + 高亮**：AI 回复中的每个代码块右上角悬停出现复制按钮，并用页面已加载的 Prism 做语法高亮
- **智能滚动**：流式输出时仅在用户本就位于底部附近才自动跟随，向上翻阅不再被强行拉回
- **思考提示 i18n**：“正在思考”走语言系统（原为硬编码中文）；新增 5 组中英操作文案
- 实测：设置面板独占布局 ✓、配置保存 ✓、错误路径+重试按钮 ✓（假地址触发 502 验证）
...
### 🎓 学习中心 52 章全部完成渐进式入门模板改造（本次大版本的核心）
...
- **标杆章重写**：`prog-01-02 控制流与函数` 按渐进式入门风格重写——"第 N 步"小步结构、每个概念最小可运行例子+运行结果+逐行拆解、安全场景串联（弱密码检测器贯穿全章）、常见坑速查表、3 道折叠答案练习、章末小结表

...
### 🚀 v2.8.0（2026-09-17）— 最新版本 / 全量里程碑

- 🏆 **全平台九大核心体系 100% 改造完工**：课时规模从 52 讲跃升至 **171 讲（342 篇中英双语教材）**，覆盖 Web 安全、渗透测试、网络基础、现代密码学、恶意代码分析、云原生与容器安全、蓝队 DFIR 取证、系统编程基础与 CTF 实战攻防 9 大学科方向。
- 🎯 **全覆盖随堂 Checkpoint 评测 (342 组)**：每节微课均配有 2 道单选自测题，实时判定正误，提供机制原理解析，并支持一键呼叫「🤖 AI 导师」进行个性化错题辅导。
- 🛠️ **6 大交互安全武器库与沙箱**：内置 SQL 注入对比沙箱、XSS 实体演练场、命令注入隔离分析仪、密码学哈希雪崩分析仪、五维技能雷达图与专属实验抽屉。
- 🎮 **黑客段位与游戏化成长体系**：5 阶段位演化（LV.1 脚本小子 → LV.5 赛博主宰），多维 EXP 计算、连续答题加成与勋章解锁。
- 🌐 **赛博终端品牌美学与全量双语**：首页打字机与 Hero 大标题严格保持纯英文黑客终端质感（`> 171 chapters. Zero to hero.`），全站 100% 中英双语一键平滑切换。
- 📱 **PWA 渐进式离线支持**：内置 `manifest.json` 与 Service Worker 资源缓存，支持离线断网学习与独立应用安装。
- 🔒 **严苛质量保障门禁**：114 项单元测试全过、21 道 CTF 真实数学可解、342 篇微课合规（342/342 OK）、零隐私泄漏。

### 📦 v2.7.6（2026-08-30）

- 🗂 **AI 历史改为全面板抽屉** — 搜索、时间分组（今天/7天内/更早）、重命名、两段式删除、清空全部
- 🤖 **AI 导师体验升级** — ■ 停止生成、消息操作条（复制/重新生成）、代码块复制 + Prism 高亮、智能平滑滚动
- 🔒 **隐私扫描门禁入 CI** — 扫描盘符路径、私钥块、AKIA 密钥与真实邮箱，确保开源零泄露
- 🚩 **28 道 CTF 全部真实可解** — 真实 RSA 参数、可解码密文、内嵌取证产物、2 个新服务端模拟器；每道答案均可从题目本身推导
- 📊 **文章元信息条** — 每章显示难度星级 + 前置章节提示
- 🐛 **问题修复** — 修复 submitFlag 提交问题、28 题答案错位、CSP void 内联与 content.js 死赋值压缩

<details><summary><b>查看历史版本更新记录（v2.7.0 ~ v2.6.0）</b></summary>

#### v2.7.0（2026-08-28）
- 📚 **52 章全部完成渐进式入门模板改造** — 每章标配“本章你将学会”目标框 + 分步例子 + 3 道折叠答案练习 + 章末小结表（中英双语）
- 🗂 **模块顺序优化** — 编程 → 网络 → 密码学 → Web → 渗透 → 恶意软件 → CTF（消除 TLS 先于 HTTP 的依赖倒置）
- 🔢 **侧边栏章节编号** — 每章显示 01. 02. 03. … 进度感可视化

#### v2.6.0（2026-08-27）
- 🔒 **服务器仅回环监听** — 仅绑定 127.0.0.1，防范局域网越权调用
- 🛡️ **DNS 重绑定防护** — 校验 Host 请求头，隔绝跨站劫持
- 🚫 **移除 CORS 权限** — 浏览器其他网页无法非法驱动本服务接口
- 🔑 **答案哈希化** — CTF Flag 采用 SHA-256 哈希校验，仓库零明文
- 📦 **静态文件黑名单** — 彻底阻断 `.git`、源码与服务配置等元数据穿透

</details>

> 📋 [查看更早详细版本历史请访问 CHANGELOG.md →](versions/CHANGELOG.md)
